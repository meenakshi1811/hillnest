...
<body class="min-h-screen flex flex-col bg-cream">
    <header class="sticky top-0 z-50 bg-cream/96 shadow-[0_14px_36px_rgba(61,41,20,0.10)] backdrop-blur-xl">
        <div class="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
            <div class="flex min-h-[102px] items-center justify-between gap-4 py-3 lg:min-h-[124px]">
                <x-logo />

                <nav class="hidden lg:flex items-center gap-1 rounded-2xl bg-white/90 p-1.5 text-[15px] font-medium text-brand-light shadow-[0_10px_28px_rgba(61,41,20,0.08)] ring-1 ring-hill-200/80">
                    <a href="{{ route('home') }}" class="rounded-xl px-4 py-2.5 transition hover:bg-gold-bg hover:text-brand {{ request()->routeIs('home') ? 'bg-brand text-white shadow-sm hover:bg-brand hover:text-white' : '' }}">Home</a>
                    <a href="{{ route('shop.index') }}" class="rounded-xl px-4 py-2.5 transition hover:bg-gold-bg hover:text-brand {{ request()->routeIs('shop.*') ? 'bg-brand text-white shadow-sm hover:bg-brand hover:text-white' : '' }}">Shop</a>
                    <a href="{{ route('about') }}" class="rounded-xl px-4 py-2.5 transition hover:bg-gold-bg hover:text-brand {{ request()->routeIs('about') ? 'bg-brand text-white shadow-sm hover:bg-brand hover:text-white' : '' }}">Our Story</a>
                    <a href="{{ route('shop.index') }}#collection" class="rounded-xl px-4 py-2.5 transition hover:bg-gold-bg hover:text-brand">Bilona Ghee</a>
                </nav>

                <div class="flex items-center gap-2 sm:gap-3">
                    <a href="{{ route('shop.index') }}" class="hidden lg:inline-flex items-center gap-2 rounded-xl bg-white px-3.5 py-2.5 text-[15px] font-medium text-brand-light shadow-sm ring-1 ring-hill-200/80 transition hover:bg-gold-bg hover:text-brand" aria-label="Search products">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <span>Search</span>
                    </a>

                    @auth
                        <a href="{{ route('account.orders') }}" class="hidden lg:inline-flex items-center rounded-xl bg-white px-3.5 py-2.5 text-[15px] font-medium text-brand-light shadow-sm ring-1 ring-hill-200/80 transition hover:bg-gold-bg hover:text-brand">My Account</a>
                    @else
                        <a href="{{ route('login') }}" class="hidden lg:inline-flex items-center rounded-xl bg-white px-3.5 py-2.5 text-[15px] font-medium text-brand-light shadow-sm ring-1 ring-hill-200/80 transition hover:bg-gold-bg hover:text-brand">Log in</a>
                    @endauth

                    <a href="{{ route('cart.index') }}" class="relative inline-flex items-center gap-2 rounded-xl bg-brand px-3.5 py-2.5 text-white shadow-lg shadow-brand/15 transition hover:bg-gold sm:px-4" aria-label="Cart">
                        ...
                        <span class="hidden sm:inline text-[15px] font-medium">Cart</span>
                        @if(($cartCount ?? 0) > 0)
                    ...
                    <button type="button" id="mobile-menu-btn" class="lg:hidden inline-flex h-12 w-12 items-center justify-center rounded-xl bg-white text-brand shadow-sm ring-1 ring-hill-200/80 transition hover:bg-gold-bg" aria-label="Menu" aria-controls="mobile-menu" aria-expanded="false">
        ...
        <div id="mobile-menu" class="hidden lg:hidden border-t border-hill-200/80 bg-cream/98 px-4 py-5 shadow-inner">
            <nav class="mx-auto flex max-w-[1400px] flex-col gap-3 text-base font-medium text-brand sm:px-2">
